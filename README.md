...
- **Expandable cards** — collapse or expand each solution to reveal its subtasks.
- **Clickable status cycling** — click any subtask status to advance it through **To-do → In progress → Done → Blocked** and back to To-do.
...
- **Export JSON** and **Export CSV** — download the full catalog (with your current statuses) as `voltdown-solutions.json` or `voltdown-solutions.csv`.
...
The **impact ratings**, **cost ratings**, and **default statuses** are **illustrative seeded values**, generated to make the tracker usable and visually meaningful out of the box. They are **not** sourced estimates, expert assessments, or real project data, and they should not be read as evidence about which solutions are actually most effective, cheapest, or already underway. Treat the catalog as a structured starting point for thinking and planning — the ratings and statuses are there so the dashboard and filters have something to work with, not as factual claims.
