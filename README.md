...
- **Cards / Table view toggle** — a **▦ Cards / ▤ Table** switch in the controls flips between the card grid and a full table of every solution (columns: **#, Category, Solution, Impact, Feasibility, Est. cost USD, Est. cost PHP, Subtasks, Progress**). The table is sortable and filterable through the **same search box, category chips, status filter, and sort dropdown** as the cards — they all share one state — and clicking a row (or its **#id** button) opens the **same right-side detail panel**.
- **Master–detail panel** — click a card or table row (or focus a card and press **Enter**/**Space**) to slide open a right-side detail panel showing the solution's full subtask list, each subtask's estimated cost, the rolled-up estimated total, and live progress. Close it with the **✕** button, the **Esc** key, or by clicking the backdrop. Cards are keyboard-accessible (`role="button"`, focusable, with an ARIA label) and the active card stays highlighted while the panel is open. The panel is a focus-managed dialog — see **Accessibility** below.
- **Dual-currency costs (USD + approximate PHP)** — every cost is shown in **USD next to an approximate Philippine-peso (₱) figure** (e.g. `$4.4M · ₱256M`), converted with a single illustrative constant of **~58.5 ₱ per US$** (not a live quote). This applies on the cards, in the detail panel (both the per-subtask cost and the rolled-up total), in the table's **Est. cost USD** / **Est. cost PHP** columns, and in the JSON/CSV exports (new `*_usd` / `*_php` columns).
- **Per-subtask cost + per-solution total** — every subtask carries an illustrative estimated cost (USD, with the matching approximate PHP), and each solution rolls them up into an **estimated total cost** shown as a pill on the card and broken out in the detail panel.
...
- **Export JSON** and **Export CSV** — download the full catalog (with your current statuses, **the cost columns** — per-subtask estimated cost and the per-solution rolled-up estimated total, each in **USD and approximate PHP** via the new `*_usd` / `*_php` columns — and the **feasibility score and band**) as `voltdown-solutions.json` or `voltdown-solutions.csv`.
...
The **Philippine-peso (₱) figures** shown next to every USD cost are likewise **illustrative**: they are derived by multiplying the (already illustrative) USD figure by a **single static rate of ~58.5 ₱ per US$**. That rate is a fixed constant baked into the app — it is **not a live exchange rate, not a quote, and not updated automatically** — so the peso amounts are only a rough at-a-glance conversion and should not be treated as accurate currency values.
